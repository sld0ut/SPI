...
module SPI_Master (
  ...
  always @(posedge i_Clk or negedge i_Rst_L)
    if (~i_Rst_L) begin
		...
		if (i_TX_DV) begin
			...
			o_TX_Ready      <= 1'b0;
			r_SPI_Clk_Edges <= (i_Bitlen+1)*2;  // Total # edges in one byte ALWAYS 16
  ...
  always @(posedge i_Clk or negedge i_Rst_L)
    if (~i_Rst_L) begin
		o_SPI_MOSI     <= 1'b0;
		r_TX_Bit_Count <= 'h0; // send MSb first
    end else begin
		//Fix DFT Violation
		if (i_TX_DV) begin
			r_TX_Bit_Count <= i_Bitlen; // send MSb first
		end
		// If ready is high, reset bit counts to default
		if (o_TX_Ready) begin
			r_TX_Bit_Count <= i_Bitlen;
		end else if (r_TX_DV & ~w_CPHA) begin
			// Catch the case where we start transaction and CPHA = 0
		 	o_SPI_MOSI     <= r_TX_Byte[i_Bitlen];
		 	r_TX_Bit_Count <= i_Bitlen - 1'b1; 
